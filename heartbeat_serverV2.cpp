...
void hblisten (int clientSocket){
 ...
 cout << "client key " << string(buf, 0, clienthblisten) << " successful heartbeat " << servtime << endl;       
}
 
int main()
{
    
    string serverconfig = ".serverconfig";
    ifstream fin(serverconfig.c_str());
        if(!fin) {
                cout << "Server config file not found" << ".\n";
                cout << "Initializing setup" << ".\n";
                string interactiveinput;
                ofstream settingfile(".serverconfig");
                cout << "Please Enter listening port ";
                getline (cin, interactiveinput);
                settingfile << interactiveinput << endl;
                cout << "Please enter pre-shared key ";
                getline (cin, interactiveinput);
                settingfile << interactiveinput << endl;
                settingfile << "0" << endl;
                settingfile.close();
                cout << "Configuration file .serverconfig created" << ".\n";
                cout << "Default Log level set to 0" << ".\n";
        }
       
   //pull configs from server config file   
   ifstream infile;
     string SPORT;
     //string SPORT, PSK, LL;
        infile.open(".serverconfig");
